...
import { ShoppingBag, User, Home, Package, Menu, X, LogOut } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center space-x-2">
            ...
            {isLoggedIn ? (
              <button
                ...
                className="flex items-center gap-2 ml-4 px-4 py-2 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl transform"
              >
        ...
        <div className={`md:hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
          <div className="px-2 pt-2 pb-6 space-y-2 bg-black/10 rounded-2xl mt-2 backdrop-blur-md border border-white/10">
            ...
            {/* Mobile Dynamic Login/Logout Button */}
            {isLoggedIn ? (
              <button
                onClick={handleLogout}
                className="flex items-center gap-3 px-4 py-3 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition-all duration-300 mt-4 w-full"
              >
                <LogOut className="w-5 h-5" />
                Logout
              </button>
